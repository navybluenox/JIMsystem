...
var Database = function () {
    ...
    _createClass(Database, [{
        ...
        value: function loadData(dataName) {
            ...
            this.loading.filter(function (v) {
                return v != dataName;
            });
        ...
        value: function reloadData(dataName) {
            ...
            while (dataNew.length) {
                ...
                index = dataNow.findIndex(function (p) {
                    return p.getValue("id") == datapiece.getValue("id");
                });
                if (index != -1) {
            ...
            dataNow.forEach(function (p) {
                var deletePiece = this.cache[dataName].data.find(function (p1) {
                    return p1.getValue("id") == p.getValue("id");
                });
                deletePiece.deleteValues();
            });
            this.cache[dataName].data = this.cache[dataName].data.filter(function (obj) {
        ...
        key: "getColumn",
        value: function getColumn(dataName) {
        ...
        value: function getDatabaseInfo(dataName) {
            //TODO
            var list = [{ dataName: "user", classObj: User, fileId: _fileId.database.user, column: [{ name: "id", type: "string", defaultValue: "" }, { name: "sort", type: "number", defaultValue: 0 }, { name: "name_last", type: "string", defaultValue: "" }, { name: "name_first", type: "string", defaultValue: "" }, { name: "name_last_phonetic", type: "string", defaultValue: "" }] }];
...
var Datapiece = function () {
    ...
    _createClass(Datapiece, [{
        ...
        value: function setValue(columnName, value) {
            if (this.getColumn().map(function (v) {
                return v.name;
            }).inArray(columnName)) {
        ...
        key: "deleteValues",
        value: function deleteValues() {
            delete this.data;
        }
    }, {
        key: "deleteValue",
        value: function deleteValue(columnName, setDefault) {
            if (setDefault == null || setDefault) {
                var dv = this.getColumn().find(function (v) {
                    return v.name = columnName;
                }).defaultValue;
                if (dv) {
