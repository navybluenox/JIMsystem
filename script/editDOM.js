...
function createTable(data,parent,callback,option){
    ...
    $(parent).children("table thead").append(headerPattern.map(function(arr){
    ...
    data.forEach(function(dp,rowIndex){
        ...
        colList.forEach(function(col,colIndex){
            ...
            callback({rowData:dp,el:pJqObj.children("td").eq(colIndex)[0],value:value,key:col});
        })
    })
    ...
    if(option.leftColumn !== undefined){
        ...
        $(parent).children("table thead tr").eq(0).prepend((new Array(addedRowNum+1)).join("<td rowSpan='0'></td>"));
        $(parent).children("table tbody tr").prepend((new Array(addedRowNum+1)).join("<td></td>"));
