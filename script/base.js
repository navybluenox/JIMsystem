...
var LocalDate = (function(){
    return class LocalDate {
        ...
        static getStandardTime(){
            return new Date(_val.config.getValue("content.base.standardTime"));
        }
        static getTimeUnit(){
            return _val.config.getValue("content.workAssign.timeUnit");
        }
        static getTimeUnitAsConverted(unitName){
            var unitValue = [1,1000,60000,3600000,86400000][["millsecond","second","minute","hour","day"].findIndex(function(v){return v === unitName})];
            return LocalDate.getTimeUnit() / unitValue;
        }
        static getOpenTime(day,kind){
            return _val.config.getOpenTime(day,kind);
        }
        static getWorkTime(day,kind){
            return _val.config.getWorkTime(day,kind);
        }
        copy(){
        ...
        toString(option){
            ...
            return [
                ...
                option.hideHour ? "" : this.getHours() + "時",
                option.hideMinute ? "" : this.getMinutes() + "分",
                option.hideSecond ? "" : this.getSeconds() + "秒",
                option.hideMillsecond ? "" : this.getMillseconds() + "ミリ秒"                
            ].join("");
        }
    }
})();
