...
class SystemConfig extends Datapiece{
    ...
    updateValues(str){
        ...
        return runServerFun("Script.updateFileToDrive",[server.getCollectionInfoByName(dataName).getDatapieceRelated("fileInfoId","fileInfo").getValue("fileId"),JSON.stringify(systemConfigs)]).then(() => {
            return server.reloadData(dataName);
        });
    }
}
