...
var Datapiece = (function(){
    ...
    return class Datapiece {
        ...
        static getShiftTableAsSpreadsheetSetting(that,dataName,start,end,rowIndex,leftOffset){
            rowIndex = rowIndex || 0;
            ...
            return rows.map((_row,rowOffset) => {
                ...
                row = row.reduce((prev,curt) =>  prev.concat(curt),[]);
...
class SystemConfig extends Datapiece{
    ...
    getWorkTime(day,kind){
        if(kind == "start" || kind == "end"){
            let localDateObj = this.getValue("content.workAssign.workTime")[day - this.getWorkStartDay()];
            return (new LocalDate(localDateObj[kind])).addDays(day);
        }else{
    ...
    updateValues(str){
        var newValue;
        var server = Datapiece.getServer();
        ...
        try{
            ...
            throw new Error([
                "Cannot parse a string-type value as JSON format.",
                "@ SystemConfig.prototype.updateValues"
            ].join("\n"));
        }
        var systemConfigs = server.getData(dataName);
        var targetIndex = systemConfigs.findIndex(systemConfig => {return systemConfig.getValue("modeName") === newValue.modeName});
        if(targetIndex === -1){
            throw new Error([
                    "Such a modeName does not exist.",
                    "@ SystemConfig.prototype.updateValues"
            ].join("\n"))
        }
        systemConfigs[targetIndex] = newValue;
        var la = new LoadingAlert();
        return runServerFun("Script.updateFileToDrive",[
            server.getCollectionInfoByName(dataName).getDatapieceRelated("fileInfoId","fileInfo").getValue("fileId"),
            JSON.stringify({"data":systemConfigs,"version":server.getVersion(dataName) + 1,"updated":(new Date().toISOString())},null,2)
        ]).then(() => {
            return server.reloadData(dataName).then(()=>{la.remove();});
        });
    }
    static getPropertyCaption(){
        //CollectionInfoに直書きしたくないので、仕方なくここに直書き
        return {
            "modeName":"実行モードの名前 基本的に「祭の略称+期」",
            "content.kind":"祭の種類",
            "content.nth":"期",
            "content.base":"基本情報の設定群",
            "content.base.standardTime":"0日目0時0分のグリニッジ基準時のISO文字列 （Date.prototype.ISOString()で出力されるものと同形式）",
            "content.base.openTime":"企画公開時間 配列で複数記入 1日目から全て指定する",
            "content.base.openTime.[Number].start":"企画公開開始時間",
            "content.base.openTime.[Number].start.hour":"企画公開開始時間_時間",
            "content.base.openTime.[Number].start.minute":"企画公開開始時間_分",
            "content.base.openTime.[Number].end":"企画公開終了時間",
            "content.base.fileId_fileInfo":"fileInfo.json",
            "content.workAssign":"人割に関する設定群",
            "content.workAssign.timeUnit":"人割の1単位の時間 単位はミリ秒",
            "content.workAssign.workStart":"人割開始日時 人割が開始する日数 「n日目」のnを設定する",
            "content.workAssign.workTime":"人割時間 人割として管理する時間 この範囲外だと人割表に載らない 表記法はcontent.base.openTimeと同じ 配列の0番目は人割が開始される日のもの（not0日目）",
            "content.shiftTable":"人割表出力の設定群",
            "content.shiftTable.insertHeader":"人割表に表示するヘッダー（時間軸）の挿入位置の設定",
            "content.shiftTable.insertHeader.leaderCode":"局長の担当略称 配列で複数記入",
            "content.shiftTable.insertHeader.azusaSendName":"その他、老人などヘッダーを入れる直後のAZUSA標準配送名 配列で複数記入"
        };
    }
}
...
class User extends Datapiece{
    ...
    getIncharge(isAll,getFromData){
        ...
        if(this._incharge === undefined || getFromData)  this._incharge = Datapiece.getServer().getDataById(this.getValue("inchargeId"),"incharge");
        return isAll ? this._incharge.slice() : this._incharge.filter(incharge => incharge.isPresentTerm());
    }
    getInchargeForDisplay(option){
        option = $.extend({"hideOldInchargeAtNotRojin":true},option === undefined ? {} : option);

        return this.getIncharge(true)
            .filter(incharge => !option.hideOldInchargeAtNotRojin || this.getValue("isRojin") || incharge.isPresentTerm())
            .filter(incharge => !incharge.isInvisible() && !inArray(this.getValue("hideIncharge"),incharge.getValue("_id")));
    }
    getBackgroundColor(){
        return !!this.getValue("backgroundColor") ? this.getValue("backgroundColor") : UserGroup.getColorByUserId(this.getValue("_id"),"background");
    }
    getFontColor(){
        return !!this.getValue("fontColor") ? this.getValue("fontColor") :  UserGroup.getColorByUserId(this.getValue("_id"),"font");
    }
...
class WorkList extends Datapiece{
    ...
    getNumberDetails(){
        return this.getValue("@detail").map(obj => {
            obj.end = obj.start.copy().addTimeUnit(obj.number.length);
            return obj;
        });
    }
    getBackgroundColor(){
    ...
    getShiftTableAsSpreadsheetSetting(timeSetting,rowIndex,leftOffset){
        var start,end;
        if(typeof timeSetting === "number"){
            let detailObj = this.getValue("@detail")[timeSetting];
            if(detailObj === undefined)  throw new Error();
            start = detailObj.start;
            end = start.copy().addTimeUnit(detailObj.number.length);
        }else if(typeof timeSetting === "object" && timeSetting !== null && timeSetting.start !== undefined && timeSetting.end !== undefined){
            start = new LocalDate(timeSetting.start);
            end = new LocalDate(timeSetting.end);
        }else{
            throw new Error();
        }
        return Datapiece.getShiftTableAsSpreadsheetSetting(this,this.getDataName(),start,end,rowIndex,leftOffset);
...
class Incharge extends Datapiece{
    ...
    getMemberIncharges(includeNotEndChildren,eliminatedIds){
        ...
        var member = this.getValue("member")
            ...
            .map((incharge,i,self) => incharge.getMemberIncharges(includeNotEndChildren,self.map(incharge => incharge.getValue("_id")).concat(eliminatedIds)))
            .reduce((prev,curt) => prev.concat(curt),[this]);
            
        return member.filter(incharge => includeNotEndChildren || incharge.isEndChild()).filter((v,i,s) => s.findIndex(incharge => incharge.getValue("_id") === v.getValue("_id")) === i);
    ...
    static getAllOfAllParents(putPresentTermTop){
        ...
        return Datapiece.getServer().getData("incharge").filter(incharge => incharge.isAllParent())
            .sort((a,b) => {
                if(putPresentTermTop){
                    if(a.isPresentTerm()) return -1;
                    if(b.isPresentTerm()) return 1;                            
                }
                return arr.indexOf(a.getName()) - arr.indexOf(b.getName());
            });
    }
