...
class SystemConfig extends Datapiece{
    ...
    updateValues(str){
        ...
        var targetIndex = systemConfigs.find(systemConfig => {return systemConfig.getValue("modeName") === newValue.modeName});
        if(targetIndex === -1){
            throw new Error([
        ...
        /*var la = new LoadingAlert();
        return runServerFun("Script.updateFileToDrive",[server.getCollectionInfoByName(dataName).getDatapieceRelated("fileInfoId","fileInfo").getValue("fileId"),JSON.stringify(systemConfigs,null,2)]).then(() => {
            return server.reloadData(dataName).then(()=>{la.remove();});
        });*/

        //TODO
        console.log("before",JSON.stringify(server.getData(dataName),null,2));
        console.log("after",JSON.stringify(systemConfigs,null,2));
