$(function(){
    ...
    _val.pageFun.editWorkList = {
        onload:function(){
            ...
            tr1.children().not(":last-child").remove();
            tr2.children().remove();
            tr1.prepend(repeatString("<td></td>",numberArray.length));
            tr2.append(repeatString("<td></td>",numberArray.length));

            (function(){
                var a = numberArray.slice();
                var num,td1,td2,time,numIndex;
                while((num = a.pop()) !== underfined){
                    numIndex = a.length;
            ...
            var td1s = tr1.children().not(":last-child");
            var td2s = tr1.children();
