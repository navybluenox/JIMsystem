    ...
    _val.pageFun.editDatabase = {
        ...
        showTable:function(){
            ...
            promise.then(function(){
                ...
                //配列のデータのチェックボックスに応じて、該当する行のテキストボックスを無効にする
                $table.children("tbody").on("click",'tr td input[type="checkbox"][name^="table-removearrayvalue-"]',function(e){
                    var target = $(e.currentTarget);
                    var names = target.attr("name").split("-");
                    var inputs = $table.find('input[name^="' + ["table","content",names[2],names[3],""].join("-") + '"]');
        ...
        updateData:function(){
            ...
            //add
            separateAssembleOfQueues('input[type="text"][name^="table-content-new"]',"addData");
            //change
            separateAssembleOfQueues('input[type="text"][name^="table-content-"]:not([name^="table-content-new"])',"changeData");

            function separateAssembleOfQueues(selector,method){
                var queues = {};
