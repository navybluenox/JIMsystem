$(function(){
    ...
    _val.pageFun.editDatabase = {
        ...
        showTable:function(){
            ...
            if(dataName === "blank") return;
            $(result).children().remove();
          
            _val.server.loadData(dataName).then(function(){
                var dataArr = _val.server.getData(dataName,null,true,true)
                ...
                var columnObj = collInfo.getValue("column");
                ...
                function editCells(cellObj){
                    ...
                    if(columnName === "remove"){
                        ...
                        if(classof(columnObj[columnName]) === "array"){
                            var valueTable = $("<table><thead></thead><tbody></tbody></table>").appendTo(cellObj.$el);
                            var keyInfo = columnObj[columnName][0];

                            keys = (classof(keyInfo) === "object" ? Object.keys(keyInfo) : [""]);
                            valueTable.children("tbody").data({"length":cellObj.value.length,"keys":keys,"_id":cellObj.rowData._id,"column":columnName});
                            ...
                            cellObj.value.forEach(function(v,vIndex){
                                ...
                                $('<input type="checkbox">').appendTo(thisRow.children("td").eq(0))
                                    .attr("name",["table","removearrayvalue",cellObj.rowData._id,columnName,vIndex].join("-"));
                                keys.forEach(function(key,keyIndex){
                                    var thisCell = thisRow.children("td").eq(keyIndex + 1);
                                    var input = $('<input type="button">').appendTo(thisCell);
                                    var names = ["table","content",cellObj.rowData._id,columnName,vIndex];
                            ...
                            input.val(castIntoString(cellObj.value)).attr("name",["table","content",cellObj.rowData._id,columnName].join("-"));
                        }
                    }
                }

                //theadにカラム名（日本語）を追加
                var columnNameObj = collInfo.getValue("columnName");
                $("<tr>" + columns.map(function(column,colIndex){
                    if(colIndex === 0 || colIndex === 1) return "<th></th>";
                    var type = columnObj[column];
                    var name = columnNameObj[column];
                    switch(classof(type)){
                        case "string":
                            return "<th>" + name + "</th>"
                        case "array":
                            if(classof(type[0]) === "object"){
                                return '<th style="word-break:break-all;"><p style="margin:0;">' + name[0]._name + '</p><p style="margin:0;">' + Object.keys(type[0]).map(function(key){return key + ":" + name[0][key]}).join(", ") + "</p></th>"
                            }else{
                                return "<th>" + name[0] + "</th>"
                            }
                        default:
