    ...
    _val.pageFun.assignWork = {
        onload:function(){
            ...
            form.find('[name="shiftTableUser"]').siblings("div").on({"mouseenter":function(e){
                ...
                if(target.find("div.overlay").length > 0)  return;
                var wrapper = $("<div></div>").css({"background":"#FFC21A","opacity":"0.5","z-index":"1"}).addClass("overlay");
                target.children("div").wrap(wrapper);
            },"mouseleave":function(e){
                var target = $(e.currentTarget);
                if(target.children("div:not(.freezeOverlay)").length > 0){
                    target.find("div.overlay div").unwrap();
                }
            ...
            form.find('[name="shiftTableUser"]').siblings("div").on("click","td.shiftTableContent",function(e){
                ...
                if(div.find("td.selectedCell").length === 1 && div.find("td.selectedCell").data("workIndex") === target.data("workIndex")){
                    ...
                    div.find("td.shiftTableContent").filter(function(index,_el){
                        ...
                        var el = $(_el);
                        var wrapper = $("<div></div>").css({/*"background":"#FFC21A","opacity":"0.5",*/"z-index":"1"}).addClass("overlay");
                        el.children("div").wrap(wrapper);
                        el.addClass("selectedCell");
                    });
                }else{
                    //それ以外のクリックで1つ目のセルを選択
                    div.find("td.selectedCell").removeClass("selectedCell").find("div.overlay div").unwrap();

                    var target = $(e.currentTarget);
                    var wrapper = $("<div></div>").css({/*"background":"#FFC21A","opacity":"0.5",*/"z-index":"1"}).addClass("overlay");
                    wrapper.css({"border":"2px solid #000000"}).addClass("freezeOverlay");
                    target.children("div").wrap(wrapper);
                    }
                    target.addClass("selectedCell");
                div.find("td.selectedCell").children("div").addClass("freezeOverlay").css({"border":"2px solid #000000","border-left-width":"0","border-right-width":"0"});
                div.find("td.selectedCell").children("div").last().css("border-right-width","2px");
                div.find("td.selectedCell").children("div").first().css("border-left-width","2px");
            });
