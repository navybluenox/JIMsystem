    ...
    _val.pageFun.handleSpreadsheet = {
        onload:function(){
            ...
            timeInfoList.forEach(function(timeInfo){
                ...
                //borderSetting.push({"range":{"top":topOffset,"left":leftOffset,"height":users.length + indexOfHeader.length,"width":contentWidth},"border":{"style":"solid","horizontal":true}});
                ...
                groupUsers.forEach(function(obj,objIndex){
                    var users = obj.user;
                    var groupIndex = obj.index;
                    table.push(makeHeader(topOffset + groupIndex + objIndex));

                    users.forEach(function(user,rowIndex){
                        var ret = user.getShiftTableAsSpreadsheetSetting(start,end,rowIndex + topOffset + groupIndex + objIndex + 1,leftOffset);
