    ...
    _val.pageFun.handleSpreadsheet = {
        onload:function(){
            ...
            var constValue = {
                "leftOffset":0,
                "topOffset":1,
                "leftOffset_workList":1,
                "topOffset_workList":0,
                "leftOffset_detail":0,
                "topOffset_detail":1
            };
            Server.handlePropertiesService(version_propertyKey,"script","get").then(function(v){version = (v[version_propertyKey] === undefined ? 0 : +v[version_propertyKey]);})
            .then(function(){
                ...
                sheetSettings.forEach(function(tableDataObj){
                    var table = [];
                    var spreadsheet = new Spreadsheet("shiftTableWork",tableDataObj.sheetName,table);
                    var sheetName = tableDataObj.sheetName;
                    var workLists = tableDataObj.workLists;
                    var mergeSetting = [];
                    var borderSetting = [];
                    var sizeSetting = [];
                    /*var start = timeInfo.start;
                    ...
                    var rowIndex_now = constValue.topOffset;

                    workLists.forEach(workList => {
                        //workListごとのヘッダー
                        var leftOffset_workList = constValue.leftOffset + constValue.leftOffset_workList;
                        var topOffset_workList = rowIndex_now + constValue.topOffset_workList;
                        var width_workList = constValue.leftOffset_workList;
                        var height_workList = constValue.topOffset_workList;

                        workList.getNumberDetails().forEach(detail => {
                            var leftOffset_detail = leftOffset_workList + constValue.leftOffset_detail;
                            var topOffset_detail = rowIndex_now + constValue.topOffset_detail;
                            var width_detail = detail.number.length;
                            var height_detail = detail.number.reduce((prev,curt) => Math.max(prev,curt),0);
                            width_workList = Math.max(width_workList,width_detail + constValue.leftOffset_detail);
                            height_workList += height_detail;
                        });
