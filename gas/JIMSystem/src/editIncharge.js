$(() => {
    ...
    _val.pageFun.editIncharge = {
        onload:() => {
            _val.server.loadDataAll().then(() => {
                form.find('[name$="_cond_nth"]').append(
                    _val.server.getData("incharge")
                        ...
                        .map(incharge => '<option value="' + incharge.getName() +'">' + incharge.getName() + '</option>')
                        .join("")
                );
            });
            ...
            if(nth.val() !== ""){
                _incharge.find(incharge => incharge.isAllParent() && nth.val() === incharge.getName()).getValue("member").forEach(id => {
