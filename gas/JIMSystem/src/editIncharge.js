    ...
    _val.pageFun.editIncharge = {
        onload:() => {
            _val.server.loadDataAll().then(() => {
                ...
                form_search_cond.find('[name="nth"]').append(
                    ...
                    division.append(
                        ...
                        Incharge.getInchargesInOrder(incharge.getName(),true)
            ...
            var incharges = Incharge.getAllOfAllParents()
                .map(incharge => Incharge.getInchargesInOrder(incharge.getName(),true))
            ...
            var condEl = {};
            ["azusa","nameUser"].forEach(key => condEl[key] = form.find('[name="' + nameAttr + '_cond_' + key +'"]'));
            var users = _val.server.getData("user")
                .filter(user => {
                    if(inArray(skipIds,user.getValue("_id")))  return false;
                    return Object.keys(condEl).every(key => {
                        var value = condEl[key].val();
                        if(value === "" || value === null) return true;
                        switch(key){
            ...
            users.forEach(user => {
                result.append('<option value="' + user.getValue("_id") +'">' + user.getName() +'</option>');
            });
