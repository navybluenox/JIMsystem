$(() => {
    ...
    _val.pageFun.editIncharge = {
        onload:() => {
            ...
            var incharges = (condEl.nth === "" ? Incharge.getAllOfAllParents().map(inch => inch.getMemberIncharges()).reduce((prev,curt) => prev.concat(curt),[]) : _val.server.getDataById(condEl.nth,"incharge")[0].getMemberIncharges()).filter(incharge => {
