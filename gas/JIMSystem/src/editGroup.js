$(function(){
    ...
    _val.pageFun.editGroup = {
        onload:function(){
            ...
            var data = _val.server.getData(dataName).filter(datapiece => inArray(ids_search,datapiece.getValue("_id")));
            ...
            if(prefix === "member_search_"){
                ...
                datapieces = _val.server.getData(dataName_member).filter(datapiece => {
                    ...
                    conds.forEach(cond => {
                        ...
                        switch(cond.key){
                            case "azusaSendName":
                                value = [(dataName_member === "workGroup" ? datapiece.getDatapieceRelated("leaderId","user") : datapiece).getValue("azusaSendName")];
                                break;
                            case "inchargeCode":
                                value = (dataName_member === "workGroup" ? [datapiece.getValue("leaderIncharge")] : datapiece.getValue("inchargeCode"));
                                break;
                            case "division":
                                value = [];
                                //TODO
                                break;
                            case "name":
                                value = (dataName_member === "workGroup" ? [datapiece.getValue("name"),datapiece.getValue("nameShort")] : [datapiece.getValue("nameLast") + datapiece.getValue("nameFirst"),datapiece.getValue("nameLastPhonetic") + datapiece.getValue("nameFirstPhonetic")]);
                                break;
                        }
                        flag = flag && value.some(v => (new RegExp(cond.key)).test(v))
                    });
                    return flag;
                });
            }
