    ...
    _val.pageFun.editGroup = {
        onload:function(){
            ...
            form.find('[name="member_order"]').siblings("table").on("click",'[name^="member_order_"][type="button"]',e => {
                ...
                var datapieceId = button.attr("name").replace(/^member_order_[^_]+_(.+)$/,"$1");
            ...
            if(kind === "add" || kind === "change"){
                formNameList.filter(function(list){return list.only === undefined || list.only === getCollName();}).forEach(function(obj){
                    ...
                    if(inArray(["isColorGroup","isMemberOrderGroup","isUnitGroup"],key)){
                        ...
                        setValue[key] = values.slice().sort((a,b) => orderValues.findIndex(id => a === id) - orderValues.findIndex(id => b === id));
            ...
            var fun_fillForm = function(group){
                formNameList.filter(function(list){return list.only === undefined || list.only === getCollName();}).forEach(function(obj){
                    ...
                    if(key === "isColorGroup" || key === "isMemberOrderGroup"){
                        ...
                        form.find('[name="member_selected"]').children().remove();
        ...
        },setMemberList:function(insertSelectedIds){
            insertSelectedIds = insertSelectedIds === undefined ? [] : insertSelectedIds;
            var target_list = form.find('[name="member_list"]');
            var target_selected = form.find('[name="member_selected"]');
            var ids_selected = target_selected.find("option").map((i,el) => $(el).attr("value")).get().concat(insertSelectedIds).filter((v,i,s) => s.indexOf(v) === i);
            ...
            var ids_search = pageFun.searchMember("member_search_");
            var data = _val.server.getData(dataName);
            var data_list = data.filter(datapiece => !inArray(ids_selected,datapiece.getValue("_id")) && inArray(ids_search,datapiece.getValue("_id")));
            var data_selected = data.filter(datapiece => inArray(ids_selected,datapiece.getValue("_id")));

            [{"data":data_list,"target":target_list},{"data":data_selected,"target":target_selected}].forEach(obj => {
                obj.target.append(Datapiece.sort(obj.data,(dataName === "user" ? ["sortId"] : (dataName === "workList" ? ["leaderIncharge","nameShort"] : []))).map(function(dp){
                    if(dataName === "user"){
            ...
            var conds = form.find('[name^="' + prefix +'"]').map((i,el) => {
                el = $(el);
            ...
            if(prefix === "member_search_"){
                ...
                datapieces = _val.server.getData(dataName_member).filter(datapiece => {
                    ...
                    conds.forEach(cond => {
                        ...
                        switch(cond.key){
                            case "azusaSendName":
                                value = [(dataName_member === "workList" ? datapiece.getDatapieceRelated("leaderId","user") : datapiece).getValue("azusaSendName")];
                                break;
                            case "inchargeCode":
                                value = (dataName_member === "workList" ? [datapiece.getValue("leaderIncharge")] : datapiece.getValue("inchargeCode"));
                                break;
                            case "division":
                                value = [];
                                //TODO
                        ...
                        flag = flag && value.some(v => (new RegExp(cond.value)).test(v));
                    });
                    return flag;
                });
            }
            return datapieces.map(datapiece => datapiece.getValue("_id"));
        },moveMember:function(type,ids){
            var target_list = form.find('[name="member_list"]');
            var target_selected = form.find('[name="member_selected"]');
